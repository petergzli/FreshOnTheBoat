...
class GetForumProfiles(Resource):
  ...
  def get(self):
    ...
    for result in results:
        username = User.query.get(result.created_by).username
        resultDictionary = {'id': result.id, 'image_url': result.image_url, 'title': result.title, 'created_by': username, 'description': result.description, 'latitude': result.latitude, 'longitude': result.longitude, 'created_at': str(result.created_at), 'location': result.location}
